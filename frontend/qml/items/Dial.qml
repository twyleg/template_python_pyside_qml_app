...
Item {
	...
	Svg {
		...
		rotation: 360 * (stopwatch_model.millis / 1000.0)
	}
	...
	Svg {
		...
		rotation: 360 * (stopwatch_model.seconds / 60)
	}
	...
	Svg {
		...
		rotation: 360 * (stopwatch_model.minutes / 60)
	}
	...
	Svg {
		...
		rotation: 360 * (stopwatch_model.hours / 24)
	}
	...
	Text {
		...
		text: stopwatch_model.state !== Dial.State.RUNNING ? "start" : "stop"
	...
	Svg {
		...
		visible: stopwatch_model.state === Dial.State.PAUSED && !resetButtonMouseArea.containsPress
	}
	...
	MouseArea {
		...
		enabled: stopwatch_model.state === Dial.State.PAUSED
	...
	Timer {
		...
		interval: 500
		repeat: true
		running: stopwatch_model.state === Dial.State.PAUSED
	...
	Text {
		id: timeText
		text: String(stopwatch_model.hours).padStart(2, '0') + ":" + String(stopwatch_model.minutes).padStart(2, '0') + ":" + String(stopwatch_model.seconds).padStart(2, '0')
	...
	Text {
		id: millisText
		text: String(stopwatch_model.millis).padStart(3, '0')
